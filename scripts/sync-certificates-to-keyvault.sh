...
for externalsecret_file in $EXTERNAL_SECRET_FILES; do
    ...
    namespace=$(yq eval '.metadata.namespace' "$externalsecret_file")
    ...
    if kubectl get secret "${component_name}-tls" -n "$namespace" >/dev/null 2>&1; then
        echo "✅ Kubernetes secret exists: ${component_name}-tls"
        
        # Extract certificate and private key
        CERT_DATA=$(kubectl get secret "${component_name}-tls" -n "$namespace" -o jsonpath='{.data.tls\.crt}' | base64 -d)
        KEY_DATA=$(kubectl get secret "${component_name}-tls" -n "$namespace" -o jsonpath='{.data.tls\.key}' | base64 -d)
        ...
        echo "❌ Kubernetes secret not found: ${component_name}-tls"

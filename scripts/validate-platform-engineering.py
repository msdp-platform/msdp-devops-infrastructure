...
def validate_versions(config):
    ...
    required_components = ['backstage', 'crossplane', 'argocd']
    ...
    for component in required_components:
        ...
        print(f"✅ {component}: chart={component_versions['chart_version']}, app={component_versions['app_version']}")
...
def validate_backstage_config(config):
    ...
    if 'proxy' in app_config:
        ...
        msdp_services = ['/api/location', '/api/merchant', '/api/user', '/api/order', '/api/payment']
...
def validate_naming_conventions(naming_config):
    ...
    required_sections = ['organization', 'naming_conventions', 'platforms', 'components', 'environments']
    ...
    if 'name' not in org or org['name'] != 'msdp':
        print(f"❌ Organization name should be 'msdp', found: {org.get('name', 'missing')}")
        return False
...
def main():
    parser = argparse.ArgumentParser(description='Validate Platform Engineering Configuration')
    parser.add_argument('--environment', required=True, help='Environment to validate')
    parser.add_argument('--component', default='all', help='Component to validate (all, crossplane, backstage, argocd)')
    ...
    components_to_validate = ['crossplane', 'backstage', 'argocd'] if args.component == 'all' else [args.component]
